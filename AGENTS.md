...
- Use per-workspace `cargo check`, `cargo clippy`, `cargo fmt --check`, and `g3rs validate --rules-only` for compile and static checks.
- As soon as the CLI can run one command, it must verify its own fixture suite through `fixture3 check`.

## G3RS

- Keep `guardrail3-rs.toml` inside each Rust workspace root.
